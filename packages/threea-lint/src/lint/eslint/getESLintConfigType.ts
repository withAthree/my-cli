 ...
 * @param cwd
 * @param pkg
 * @returns threea-eslint-config/index
 * @returns threea-eslint-config/react
 * @returns threea-eslint-config/typescript/index
 * @returns threea-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
    ...
    return (
        'threea-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
    );
}
